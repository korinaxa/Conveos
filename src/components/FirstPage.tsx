...
const FirstPage: React.FC = () => {
  ...
  const handleSortByRating = () => {
    console.log("By rating", searchResults);
  ...
  useEffect(() => {
    ...
    const fetchMovieData = async () => {
      try {
        ...
        const sortedAlphabetically = [...response.data.Search].sort((a, b) => a.Title.localeCompare(b.Title));
        setSearchResults(sortedAlphabetically);

        const moviesWithoutRating: Movie[] = sortedAlphabetically;
        const promises = moviesWithoutRating.map(async (movie) => {
          const ratingResponse = await await axios.get(`http://www.omdbapi.com/?i=${movie.imdbID}&apikey=${apiKey}`);
          const imdbRating = ratingResponse.data.imdbRating;
          return { ...movie, imdbRating };
